...
"""
modifications by me (inhahe@gmail.com)
...
class IRCClient(basic.LineReceiver):
    ...
    def names(self, channel, nicks):
      """Names list received when joined a channel.  This may be called more than once before the list is complete."""
    ...
    def userLeft(self, nick, idathost, channel):
    ...
    def setNick(self, nickname):
        ...
        """
        self.nickname = nickname   #todo: take this line out?  the nick change might not go through.
    ...
    def irc_RPL_WELCOME(self, prefix, params):
        ...
        """
        self.signedOn(params[1])

    def irc_JOIN(self, prefix, params):
        """
        Called when a user joins a channel.
        """
        nick, idathost = prefix.split('!', 1)
        channel = params[-1]
    ...
    def irc_RPL_BOUNCE(self, prefix, params):
        print "bounce: ", params#debug
        self.bounce(server, port)        
    	
    def irc_RPL_ISUPPORT(self, prefix, params): 
        if params[0].startswith("Try server "):
          self.irc_RPL_BOUNCE(prefix, params)
        else:
          args = params[1:-1] 
          self.isupport(args) 
          if not hasattr(self, "supported"):  #someone may not have called __init__
            self.supported = ServerSupportedFeatures()
    ...
    def irc_RPL_LUSEROP(self, prefix, params):
        try:
            self.luserOp(int(params[1]), params[2]) #we might not always have a params[2]?
        except ValueError:
            pass
    ...
    def ctcpQuery(self, orig, dest, messages):
        ...
        # for better backward-compatibility I could
        # have used the ctcp_QueryXXXX format for the ones that
        # get called by the ctcp_PrivXXXX and ctcp_ChanXXXX calls
        # and passed self.nickname instead of None.
        # But it would have bugged me.
        ...
        for m in messages:
            ...
            if dest == self.nickname:
                args = orig, data
                genargs = orig, cmd, data
            else:
                args = orig, dest, data
                genargs = orig, dest, cmd, data
            method = getattr(self, "ctcp_%s%s" % (fnpref, cmd), None)
            
            print "fnpref, cmd: ", (fnpref, cmd)
            print "function: ", "ctcp_%s%s" % (fnpref, cmd)
            print getattr(self, "ctcp_%s%s" % (fnpref, cmd), None)
            print method
                
            if not method:
                method = getattr(self, "ctcp_%s" % fnpref)
            method(*args)

    def ctcp_ACTION(self, user, channel, data):
        pass
    def ctcp_ChanACTION(self, user, channel, data):
        self.ctcp_ACTION(user, channel, data)
    def ctcp_PrivACTION(self, user, data):
        self.ctcp_ACTION(user, data)

    def ctcp_PING(self, user, channel, data): 
        nick = string.split(user,"!")[0]
        self.ctcpMakeReply(user, [("PING", data)])
    def ctcp_PrivPING(self, user, data):
        self.ctcp_PING(orig, None, data)
    def ctcp_ChanPING(self, user, channel, data):
        self.ctcp_PING(user, channel, data)  

    def ctcp_FINGER(self, user, channel, data):
        if data is not None:
        ...
        nick = usersplit(user).group("nick")
        self.ctcpMakeReply(nick, [('FINGER', reply)])
        
    def ctcp_PrivFINGER(self, user, data):
        self.ctcp_FINGER(user, None, data)    
    def ctcp_ChanFINGER(self, user, channel, data):
        self.ctcp_FINGER(user, channel, data)

    def ctcp_VERSION(self, user, channel, data):
        if data is not None:
    ...
    def ctcp_PrivVERSION(self, user, data):
        self.ctcp_VERSION(user, None, data)

    def ctcp_ChanVERSION(self, user, channel, data):
        self.ctcp_VERSION(user, channel, data)
        
    def ctcp_SOURCE(self, user, channel, data):
        if data is not None:
    ...
    def ctcp_PrivSOURCE(self, user, data):
        self.ctcp_SOURCE(user, None, data)
    
    def ctcp_ChanSOURCE(self, user, channel, data):
        self.ctcp_SOURCE(user, channel, data)

    def ctcp_USERINFO(self, user, channel, data):
        if data is not None:
    ...
    def ctcp_PrivUSERINFO(self, user, data):
        self.ctcp_USERINFO(user, None, data)
    
    def ctcp_ChanUSERINFO(self, user, channel, data):
        self.ctcp_USERINFO(user, channel, data)

    def ctcp_CLIENTINFO(self, user, channel, data):
    ...
    def ctcp_PrivCLIENTINFO(self, user, data):
        self.ctcp_CLIENTINFO(user, None, data)
    def ctcp_ChanCLIENTINFO(self, user, channel, data):
        self.ctcp_CLIENTINFO(user, channel, data)
    
    def ctcp_ERRMSG(self, user, channel, data):
    ...
    def ctcp_PrivERRMSG(user, data):
        self.ctcp_ERRMSG(user, None, data)
    def ctcp_ChanERRMSG(self, user, channel, data):
        self.ctcp_ERRMSG(user, channel, data)

    def ctcp_TIME(self, user, channel, data):
        if data is not None:
    ...
    def ctcp_PrivTIME(self, user, data):
        self.ctcp_TIME(user, None, data)
    
    def ctcp_ChanTIME(self, user, channel, data):
        self.ctcp_TIME(user, channel, data)

    def ctcp_DCC(self, user, channel, data):
    ...
    def ctcp_PrivDCC(self, user, data):
        self.ctcp_DCC(user, None, data)
    def ctcp_ChanDCC(self, user, message, data):
        self.ctcp_DCC(user, data)
