...
class Commands:
  ...
  def part(window, text):
    if window.type == "channel":
      ...
      window.client.conn.leave(window.channel.name, text if text else None)
    else:
...
def _window_context_vars(window):
  ...
  if client:
    v['network_label'] = client.network_key or client.network or client.hostname or 'unknown'
  else:
    v['network_label'] = ''
  ...
  if hasattr(window, 'query') and window.query:
    ...
    v['query_nick'] = ''
  v['network_hostname'] = (client.hostname or '') if client else ''
  v['port'] = str(client.port) if client else ''
