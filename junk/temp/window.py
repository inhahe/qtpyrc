...
class Window(QWidget):
  ...
  def __init__(self, client):
    ...
    self.output.setFontFamily(state.config.fontfamily)
    self.vs = self.output.verticalScrollBar()
    self.cur = QTextCursor(self.output.document())

    self.input = QTextEdit(self)
    self.input.setAcceptRichText(False)
    self.input.setFixedHeight(state.config.fontheight)
  ...
  def addline_msg(self, nick, message):
  ...
  def _apply_activity_color(self):
    ...
    # Update tab color
    tabbar = state.app.mainwin.workspace.findChild(QTabBar)
    if tabbar:
      for i in range(tabbar.count()):
        # Match tab to this window's subwindow
        # QMdiArea tabs correspond to subwindow list order
        subs = state.app.mainwin.workspace.subWindowList()
...
class NicksList(QListWidget):
  ...
  def __init__(self, channelwindow):
    ...
    self.setFont(QFont(state.config.fontfamily))
...
def nickcontextmenu(window, nick, pos):
  ...
  kickAction = banAction = kickbanAction = None
  ...
  if channame:
    ...
    kickbanAction = opMenu.addAction("Kick+&ban")
    opMenu.addSeparator()
