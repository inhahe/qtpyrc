...
from PySide6.QtCore import QSocketNotifier, QObject, QTimer, QCoreApplication
...
class TwistedSocketNotifier(QSocketNotifier):
    ...
    def __init__(self, reactor, watcher, type):
        ...
        if type == QSocketNotifier.Type.Read:
            self.fn = self.read
        elif type == QSocketNotifier.Type.Write:
            self.fn = self.write
            ...
            if delay == 0.0:
                ...
                while True:
                    ...
                    self.qApp.processEvents(QEventLoop.ProcessEventsFlag.AllEvents | 
                                      QEventLoop.ProcessEventsFlag.WaitForMoreEvents,int(t*1010))
        finally:
            self.running=t
