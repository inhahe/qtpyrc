    ...
    for msg in final:
        ...
        if msg_type == 'user':
            for btype, *bdata in blocks:
                if btype == 'text':
                    parts.append('<div class="msg user">')
                    parts.append('<div class="role">You</div>')
                    parts.append('<div class="content">%s</div>' % _md_to_html(bdata[0]))
                    parts.append('</div>')
                elif btype == 'tool_result':
                    text = bdata[0]
                    if len(text) > 2000:
                        text = text[:2000] + '\n... (truncated)'
                    parts.append('<details class="tool-result">')
                    parts.append('<summary>Tool Result</summary>')
                    parts.append('<pre class="tool-output">%s</pre>' % html.escape(text))
                    parts.append('</details>')

        elif msg_type == 'assistant':
            for btype, *bdata in blocks:
                if btype == 'text':
                    parts.append('<div class="msg assistant">')
                    parts.append('<div class="role">Claude</div>')
                    parts.append('<div class="content">%s</div>' % _md_to_html(bdata[0]))
                    parts.append('</div>')
                elif btype == 'thinking':
                    parts.append('<details class="thinking">')
                    parts.append('<summary>Thinking...</summary>')
                    parts.append('<pre>%s</pre>' % html.escape(bdata[0]))
                    parts.append('</details>')
                elif btype == 'tool_use':
                    name, inp = bdata
                    summary = _tool_summary(name, inp)
                    parts.append('<details class="tool-call">')
                    parts.append('<summary>Tool: %s</summary>' % html.escape(name))
                    parts.append('<pre class="tool-input">%s</pre>' % html.escape(summary))
                    parts.append('</details>')
