...
def update_main_title():
  ...
  title = _expand_vars(fmt, variables, allow_eval=True,
                       eval_ns={'state': state})
  # Append active channel/topic info
  channel = variables.get('channel', '')
  if channel:
    net_label = variables.get('network_label', '')
    topic = variables.get('topic', '')
    suffix = ' - %s/%s' % (net_label, channel) if net_label else ' - %s' % channel
    if topic:
      # Strip mIRC formatting codes for the titlebar
      import re
      clean_topic = re.sub(r'[\x02\x03\x0F\x16\x1D\x1F]|\x03\d{0,2}(?:,\d{0,2})?', '', topic)
      suffix += ': %s' % clean_topic
    title += suffix
...
_DEFAULT_TEMPLATES = {
    'config':  ('# qtpyrc configuration\n'
                ...
                '# The active channel/topic is automatically appended.\n'
                '# Leave blank or remove for the default format.\n'
                'titlebar_format: >-\n'
                "  qtpyrc{eval(\"' - ' + ', '.join(sorted('%s (%s)' % (c.network_key or c.network or c.hostname, c.conn.nickname) for c in state.clients if c.connected)) if any(c.connected for c in state.clients) else ''\")}\n"
                '\n'
                '# Networks\n'
                '# networks:\n'
                '#   libera:\n'
