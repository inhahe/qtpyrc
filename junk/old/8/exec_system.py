...
def _exec_on_command(window, code, variables, conn):
  ...
  The exec context includes everything from _build_exec_context plus:
    vars   — dict of $-variables ($nick, $channel, $message, etc.)
    conn   — the IRCClient that received the event (may differ from window's conn)
    nick, channel, message, network — shorthand strings (without $ prefix)
  """
  ...
  if conn:
    ctx['conn'] = conn
  # Add convenient shorthand names (without the $ prefix)
  ...
  for on_name in matched_events:
    ...
    for hname, hinfo in list(hooks.items()):
      ...
      if pattern != '*' and not _mask_match(match_text, pattern):
        continue
      cmd = hinfo['command']
      # Execute — use hook's window, or fall back to active window
