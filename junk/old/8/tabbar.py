...
class TabbedWorkspace(QWidget):
  ...
  def _load_colors(self):
    ...
    if hasattr(self, '_tabbar_widget'):
      self._tabbar_widget.setStyleSheet("background-color: %s;" % self._bg.name())
    if hasattr(self, '_blank'):
      self._blank.setStyleSheet("background-color: %s;" % self._bg.name())
  ...
  def addSubWindow(self, widget):
    ...
    cfg = state.config
    initial_state = self.SKIPPED if cfg.new_tab_state == 'skipped' else self.NORMAL
  ...
  def _style_tab(self, entry):
    ...
    label.setStyleSheet(
      "QLabel { color: %s; background-color: %s; padding: 3px 6px; %s }" % (fg, bg, font_css))
  ...
  def _relayout(self):
    ...
    for item in items:
      if item is None:
        ...
        tw = fm.horizontalAdvance(item['label'].text()) + padding
