...
class ColorPickerWidget(QWidget):
  ...
  def __init__(self, initial_color=None, parent=None):
    ...
    for name, qt_color in _qt_colors.items():
      ...
      btn.setToolTip(name)
      btn.clicked.connect(lambda checked, clr=c: self.set_color(clr, add_history=True))
  ...
  def _on_slider(self, idx, val):
    ...
    if idx == self._fixed_axis:
      self._plane.set_fixed_value(val)
...
class FontPickerDialog(QDialog):
  ...
  def _on_accept(self):
    ...
    self._color_picker._add_to_history(self._fg_color)
    self._color_picker._add_to_history(self._bg_color)
    self.accept()
...
def _validate_font(cfg):
  """Check if the configured font exists.  If not, show a picker dialog."""
  db_families = {f.lower() for f in QFontDatabase.families()}
  if cfg.fontfamily.lower() in db_families:
    return  # font found, all good

  # fontheight is pixel height for the input widget; approximate a point size
