...
class Channel:
  ...
  def update_title(self):
    if state.config.show_network_in_tabs:
      net_label = self.client.network_key or self.client.network or self.client.hostname or ''
    else:
      net_label = ''
    base = "%s (%s)" % (self.name, net_label) if net_label else self.name
...
class Query:
  ...
  def update_title(self):
    if state.config.show_network_in_tabs:
      ...
      net_label = ''
    base = "%s (%s)" % (self.nick, net_label) if net_label else self.nick
    self.window.setWindowTitle(base)
