...
#dependencies:  Twisted, PyQt4, zope

#to prevent from depending on zope, put qt and twisted in sepaerate threads
#http://stackoverflow.com/questions/1051333/combing-an-external-event-loop-with-qts
#use qtthreads
#can twisted use qt threads?
#adapt irc.py to use sockets and select instead?
#if not using qthreads:
"""Only one main thread can do any GUI updates. If you do need to update the GUI from within the thread, Qt-4's queued connection signals make it easy to send data across threads and will automatically be invoked if you're using QThread; I'm not sure if they will be if you're using Python threads, although it's easy to add a parameter to connect()."""
...
import sys, re, cPickle, os, types, traceback
