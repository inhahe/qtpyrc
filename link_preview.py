...
async def _fetch_head(url, max_size=65536, timeout=5.0, proxy=''):
    ...
    try:
        ...
        async with aiohttp.ClientSession(connector=connector) as session:
            async with session.get(url, timeout=aiohttp.ClientTimeout(total=timeout),
                                   max_redirects=3,
                                   proxy=proxy or None,
                                   headers={'User-Agent': 'qtpyrc/1.0 (link preview)'}) as resp:
                if resp.status != 200:
                    return None
...
async def _fetch_head_stdlib(url, max_size=65536, timeout=5.0, proxy=''):
    ...
    def _do_fetch():
        try:
            req = urllib.request.Request(url, headers={
                'User-Agent': 'qtpyrc/1.0 (link preview)'})
...
async def fetch_preview(url, max_size=65536, timeout=5.0, proxy=''):
